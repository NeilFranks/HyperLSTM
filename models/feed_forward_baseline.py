...
class FeedForwardBaseline(SequenceWrapper):
    def __init__(self, input_size, output_size, hidden_size, n_layers, batch_size):
        super().__init__()
        self.first = vmap(nn.Linear(input_size,  hidden_size))
        self.mid = []
        for _ in range(n_layers):
            self.mid.append(vmap(nn.Linear(hidden_size, hidden_size)))
        self.final = vmap(nn.Linear(hidden_size, output_size))
